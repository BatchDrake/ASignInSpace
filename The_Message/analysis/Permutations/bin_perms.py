...
def apply_operations_and_write_file(operations, data, chunk_size, output_folder, output_file_name, degree=None):
    ...
    # If the hash is already in the dictionary and the content is the same, skip writing the file
    if result_hash in hashes:
        print(
            f"Duplicate file detected: {os.path.join(output_folder, output_file_name)} is the same as {hashes[result_hash]}. Skipping...")
        return

    # If we got here, the file is not a duplicate or a collision. Write the file.
    os.makedirs(output_folder, exist_ok=True)
    with open(os.path.join(output_folder, output_file_name), 'wb') as f:
        f.write(result)
        # Add the hash and full path to our dictionary
        hashes[result_hash] = os.path.join(output_folder, output_file_name)


if __name__ == "__main__":
    input_file_name = "data17square.bin"
